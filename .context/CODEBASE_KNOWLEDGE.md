...
### `lib/scraperStrategies.ts`
Multi-tier fallback strategies for sites that block standard fetch requests (e.g. Cloudflare, TLS fingerprinting):
- **Tier 2 — `fetchWithPuppeteer(url)`** — launches headless Chromium, navigates to URL, extracts `head.innerHTML`. Bypasses TLS fingerprinting, JS challenges, and cookie walls.
- **Tier 2 — `fetchBodyImagesWithPuppeteer(url)`** — same but extracts `<img>` tags from body via `page.evaluate()`.
- **Tier 3 — `fetchWithEnhancedHeaders(url)`** — standard fetch with additional `Sec-Fetch-*`, `Referer`, `Cache-Control` headers to mimic real browser navigation.
- **Tier 3 — `fetchBodyImagesWithEnhancedHeaders(url)`** — same pattern for body image extraction.

### `services/linkPreviewService.ts`
- `LinkPreviewService.getPreviewData()` uses a **3-tier fallback chain**: fetch → Puppeteer → enhanced headers
- `fetchHeadWithFallback()` — tries each tier in order, returns content + which tier succeeded
- `fetchBodyImagesWithFallback()` — uses the tier that succeeded for head content first, then falls back to others
- Console logs which tier was used for debugging (`[Scraper] Tier: ...`)
